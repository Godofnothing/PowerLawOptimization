...
def generate_synthetic_data(size: int, kappa: float = 1.0, nu: float = 1.0, lambda_min: float = 0.0, kappa_clamp = 0):
    ...
    # create init error
    coef  = torch.cat((torch.ones(kappa_clamp), torch.arange(1, size - kappa_clamp + 1))) ** (-(kappa+1) / 2)

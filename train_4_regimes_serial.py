...
from training import train_linearized, train_spectral_diagonal, train, train_full
...
def parse_args():
    ...
    # Run setting
    parser.add_argument('--tau', nargs='+', default=[-1.0, 1.0], type=float)
    parser.add_argument('--nn_runs', default=1, type=int)
    parser.add_argument('--linearized_runs', default=1, type=int)
    # Iteration type
...
if __name__ == '__main__':
    ...
    data = {
        ...
        'eigv' : eigv.cpu(), 
        'tau': args.tau,
        'nn_runs' : args.nn_runs,
        'linearized_runs' : args.linearized_runs
    }
    ...
    for B, lr, momentum in aggregator(args.batch_size, args.lr, args.momentum):
        ...
        for tau in args.tau:
            ...
            state, loss_curve, _ = train_spectral_diagonal(
                ...
                tau=tau, track_diag_err=False, 
            )
            loss_curves_params[f"sd/tau={tau}"] = loss_curve

        ### Train Full
        state = {'C' : deepcopy(c_full), 'J': torch.zeros_like(c_full), 'P': torch.zeros_like(c_full)}
        state, loss_curve = train_full(
            args.n_steps, state, K_emp / args.N, alpha_fn, beta_fn, batch_fn)
        loss_curves_params["full"] = loss_curve

        ### Train Linearized
        for i in range(args.linearized_runs):
            ...
            loss_curves_params[f"linearized/{i}"] = loss_curve
